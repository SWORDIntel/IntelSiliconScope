		...
		const int slot, const int func, const enum ich_chipset pch_generation)
{
	...
	msg_pinfo("Initializing PCI access for SPI controller:\n");
	msg_pinfo("  Device: %04x:%04x (vendor:device)\n", dev->vendor_id, dev->device_id);
	msg_pinfo("  Location: domain=%04x, bus=%02x, slot=%02x, func=%x\n", 
	...
	if (dev->device_id == 0x7E23) {
		...
		msg_pdbg("  Device is not Meteor Lake, ECAM should work normally\n");
	}
#else
	pci_acc->method = PCI_ACCESS_I386_TYPE1;
	msg_pinfo("ECAM not available in this build, using legacy PCI access (PCI_ACCESS_I386_TYPE1)\n");
#endif
	
	msg_pinfo("Initializing PCI access with method: %d\n", pci_acc->method);
	pci_init(pci_acc);
	msg_pinfo("PCI access initialized successfully\n");
	
	register_shutdown(enable_flash_pch100_shutdown, pci_acc);
	msg_pdbg("Registered PCI cleanup handler\n");

	msg_pinfo("Attempting to get PCI device handle...\n");
	struct pci_dev *const spi_dev = pci_get_dev(pci_acc, dev->domain, dev->bus, slot, func);
	if (!spi_dev) {
		msg_perr("Can't allocate PCI device.\n");
		msg_perr("  Tried to access: domain=%04x, bus=%02x, slot=%02x, func=%x\n",
		         dev->domain, dev->bus, slot, func);
		msg_perr("  PCI access method was: %d\n", pci_acc->method);
		pci_cleanup(pci_acc);
		return ret;
	}
	
	msg_pinfo("Successfully obtained PCI device handle\n");
	{
		uint16_t vendor_id = pci_read_word(spi_dev, 0x00);
		uint16_t device_id = pci_read_word(spi_dev, 0x02);
		msg_pinfo("  Verified device vendor:device = %04x:%04x\n", vendor_id, device_id);
		if (vendor_id == 0x8086 && device_id == 0x7E23) {
			msg_pinfo("Confirmed: Intel Meteor Lake SPI Controller (8086:7E23)\n");
		}
	}
	...
	msg_pinfo("Reading SPIBAR (SPI Base Address Register) from PCI config space...\n");
	const uint32_t phys_spibar = pci_read_long(spi_dev, PCI_BASE_ADDRESS_0) & 0xfffff000;
	msg_pinfo("  SPIBAR physical address: 0x%08"PRIx32"\n", phys_spibar);
	
	msg_pinfo("Mapping SPIBAR MMIO region (4KB)...\n");
	void *const spibar = rphysmap("SPIBAR", phys_spibar, 0x1000);
	if (spibar == ERROR_PTR) {
		msg_perr("Failed to map SPIBAR MMIO region\n");
		msg_perr("  Physical address: 0x%08"PRIx32"\n", phys_spibar);
		goto _freepci_ret;
	}
	msg_pinfo("SPIBAR mapped successfully\n");
	msg_pinfo("  Virtual address: 0x%0*" PRIxPTR "\n", PRIxPTR_WIDTH, (uintptr_t)spibar);
	msg_pinfo("  Physical address: 0x%08"PRIx32"\n", phys_spibar);

	/* This adds BUS_SPI */
	msg_pinfo("Initializing ICH SPI controller...\n");
	msg_pinfo("  PCH generation: %d\n", pch_generation);
	const int ret_spi = ich_init_spi(cfg, spibar, pch_generation);
	if (ret_spi != ERROR_FLASHROM_FATAL) {
		if (ret_bc || ret_spi) {
			ret = ERROR_FLASHROM_NONFATAL;
			msg_pwarn("SPI initialization completed with non-fatal warnings\n");
		} else {
			ret = 0;
			msg_pinfo("SPI controller initialized successfully\n");
		}
	} else {
		msg_perr("SPI initialization failed fatally\n");
	}
